 ...
 * Navigation + pharmacy admin context bootstrap only
 *
...
export default function PharmacyDashboardPage() {
  ...
  useEffect(() => {
    ...
    const pharmacyName = localStorage.getItem("PHARMACY_NAME") || "Pharmacy Admin";

    const savedLoginAt = localStorage.getItem("PHARMACY_LOGIN_AT");
    if (!savedLoginAt) {
    ...
    const adminId = pharmacyId ? `PHARMACY_ADMIN_${pharmacyId}` : "PHARMACY_ADMIN";
    localStorage.setItem("EMPLOYEE_ID", adminId);
    localStorage.setItem("EMPLOYEE_NAME", pharmacyName);
    localStorage.setItem("EMPLOYEE_EMAIL", "");
    localStorage.setItem("EMPLOYEE_ROLE", "PHARMACY_ADMIN");
    localStorage.setItem("EMPLOYEE_LOGIN_AT", localStorage.getItem("PHARMACY_LOGIN_AT") || new Date().toISOString());

    setAdminName(pharmacyName);
    setAdminEmail("");
  }, []);
  ...
  const handleLogout = async () => {
    ...
    localStorage.removeItem("PHARMACY_LOGIN_AT");
  ...
  const actionCards = [
    ...
    },
    {
      ...
      onClick: () => router.push("/pharmacy/customers"),
      cardClass:
    ...
    },
    {
      ...
      onClick: () => router.push("/pharmacy/orders?view=create"),
      cardClass:
    ...
    },
    {
      ...
      onClick: () => router.push("/pharmacy/pumps-manager"),
      cardClass:
    ...
    },
    {
      ...
      onClick: () => router.push("/pharmacy/pump-returns"),
      cardClass:
    ...
    },
    {
      ...
      onClick: () => router.push("/pharmacy/pump-maintenance"),
      cardClass:
    ...
    },
    {
      ...
      onClick: () => router.push("/pharmacy/orders?view=activity"),
      cardClass:
    ...
    },
    {
      ...
      emoji: "📄",
      link: "Open archives",
      onClick: () => router.push("/pharmacy/delivery-pdfs"),
      cardClass:
    ...
    },
    {
      ...
      emoji: "📍",
      link: "View live map",
      onClick: () => router.push("/pharmacy/driver-tracking"),
      cardClass:
      ...
      <aside className="hidden md:flex w-72 bg-[#0d1220]/90 border-r border-white/5 p-5 flex-col gap-6">
        ...
        <div className="mt-auto bg-white/5 rounded-2xl p-4 border border-white/10 space-y-3">
          <div className="space-y-1">
            <p className="text-xs font-bold text-emerald-300 tracking-widest">PHARMACY ADMIN</p>
            <p className="text-sm font-semibold text-white">{adminName}</p>
            {adminEmail && <p className="text-[11px] text-slate-400 normal-case">{adminEmail}</p>}
      ...
      <main className="flex-1 p-4 md:p-8 lg:p-10 overflow-y-auto">
        <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Full access to edit pumps, customers, orders and operations.</p>
          </div>
